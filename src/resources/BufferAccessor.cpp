...
BufferAccessor::BufferAccessor(BufferView buffer_view, const StructureType structure_type,
                               const ComponentType component_type, const size_t offset)
  : buffer_view_(std::move(buffer_view)), element_size_(sizeOf(structure_type) * sizeOf(component_type)),
    byte_offset_(offset), structure_type_(structure_type), component_type_(component_type) {
  throwIf<InitializationError>(byte_offset_ + element_size_ > buffer_view_.stride(),
                               "Bad buffer accessor (offset + element_size > BufferView.stride).");
}
